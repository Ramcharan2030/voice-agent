...
class KnowledgeBaseClient(BaseDBClient):
    ...
    async def update_document_status(
        ...
        Args:
            ...
            clear_error: Clear any previous processing error when retrying or completing
    ...
    async def reset_document_for_processing(
        self,
        document_id: int,
    ) -> Optional[KnowledgeBaseDocumentModel]:
        """Clear partial processing output before retrying a document."""
        async with self.async_session() as session:
            query = select(KnowledgeBaseDocumentModel).where(
                KnowledgeBaseDocumentModel.id == document_id
            )
            ...
            logger.info(f"Reset document {document_id} for processing retry")
            return document
