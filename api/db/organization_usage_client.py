...
class OrganizationUsageClient(BaseDBClient):
        ...
        async with self.async_session() as session:
            ...
            for run in runs:
                ...
                if run.cost_info and "total_cost_usd" in run.cost_info:
                    run_data["total_cost_usd"] = run.cost_info["total_cost_usd"]
        ...
        async with self.async_session() as session:
            ...
            daily_usage = await session.execute(
                select(
                    ...
                    func.sum(
                        WorkflowRunModel.cost_info["total_cost_usd"].as_float()
            ...
            for row in daily_usage:
                ...
                minutes = seconds / 60
                cost_usd = row.total_cost_usd or 0
    ...
    def _calculate_current_period(
        ...
        """Calculate the current usage period based on organization settings."""
