...
class PipecatEngine:
    ...
    async def _setup_llm_context(self, node: Node) -> None:
        ...
        if node.tool_uuids and self._custom_tool_manager:
            await self._ensure_mcp_sessions(node.tool_uuids)
    ...
    async def _open_mcp_sessions(self) -> None:
        ...
        await self._ensure_mcp_sessions()

    async def _ensure_mcp_sessions(
        self, tool_uuids: Optional[list[str] | set[str]] = None
    ) -> None:
        """Connect MCP-category tools on demand.

        Sessions stay alive for the call, but are opened only for the active
        node's tools so future-node MCP servers don't delay the initial greeting.
        """
        ...
        try:
            if tool_uuids is None:
                target_tool_uuids: set[str] = set()
                for node in self.workflow.nodes.values():
                    for tu in getattr(node, "tool_uuids", None) or []:
                        target_tool_uuids.add(tu)
            else:
                target_tool_uuids = set(tool_uuids)

            target_tool_uuids -= set(self._mcp_sessions)
            if not target_tool_uuids:
                return
            ...
            tools = await db_client.get_tools_by_uuids(
                list(target_tool_uuids), organization_id
            )
            sessions_to_start: list[tuple[str, McpToolSession]] = []
            for tool in tools:
                ...
                sessions_to_start.append((tool.tool_uuid, session))

            async def start_session(
                tool_uuid: str, session: McpToolSession
            ) -> tuple[str, McpToolSession]:
                await session.start()
                return tool_uuid, session

            results = await asyncio.gather(
                *[
                    start_session(tool_uuid, session)
                    for tool_uuid, session in sessions_to_start
                ],
            ...
            for result in results:
                ...
                if isinstance(result, BaseException):
                    logger.warning(
                        f"Failed to start MCP session; call proceeds without it: {result}"
                    )
                    continue
                tool_uuid, session = result
                self._mcp_sessions[tool_uuid] = session
        except Exception as e:
            logger.warning(
