...
relevant information from the local knowledge base during conversations.
...
async def retrieve_from_knowledge_base(
    ...
    Uses the configured embedding provider. If embeddings are not configured
    separately, callers can pass the resolved realtime provider settings.
    ...
    Args:
        ...
        embeddings_provider: Optional embedding provider
    ...
    try:
        ...
        if chunked_uuids is None or len(chunked_uuids) > 0:
            if not embeddings_api_key:
                raise ValueError(
                    "Embeddings API key not configured. Configure an embedding "
                    "provider or use Google realtime so its API key can be reused."
                )
