...
async def get_user(
    ...
    try:
        if SINGLE_ORGANIZATION_MODE:
            organization, org_was_created = await assign_user_to_managed_organization(
                user_model
            )
        else:
            (
...
async def create_user_configuration_with_mps_key(
    ...
    if not dograh_cloud_enabled():
        logger.info(
            "Skipping default Dograh model configuration because Dograh cloud is disabled"
        )
        return None
    ...
    async with httpx.AsyncClient() as client:
        ...
        if AUTH_PROVIDER == "local":
            ...
            if not HOSTED_MPS_SECRET_KEY:
                logger.warning(
                    "Warning: HOSTED_MPS_SECRET_KEY not set for authenticated mode"
                )
                raise ValidationError("Missing HOSTED_MPS_SECRET_KEY in non oss mode")
            ...
            response = await client.post(
                ...
                headers={"X-Secret-Key": HOSTED_MPS_SECRET_KEY},
                timeout=10.0,
            )
