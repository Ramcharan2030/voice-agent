...
class UserConfigurationValidator:
    ...
    async def validate(
        ...
        only_services: Optional[set[str]] = None,
        ...
        if only_services is not None:
            for service_name in sorted(only_services):
                service_config = getattr(configuration, service_name, None)
                status_list.extend(
                    self._validate_service(
                        service_config,
                        service_name,
                        required=service_name != "embeddings",
                    )
                )
        else:
    ...
    def _validate_service(
        ...
        provider = self._provider_value(service_config.provider)
    ...
    def _check_dograh_api_key(self, model: str, api_key: str) -> bool:
        if not dograh_cloud_enabled():
            raise ValueError(
                "Dograh cloud providers are disabled. Configure your own LLM, TTS, "
                "or STT provider instead."
            )
        if api_key.startswith("dgr"):
            raise ValueError(
                "You provided a SPX Voice API key (dgr...) instead of a service key. "
