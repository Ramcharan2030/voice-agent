...
class _OrgAttributeSpanProcessor(SpanProcessor):
    ...
    def on_start(self, span, parent_context=None):
        ...
        if org_id:
            span.set_attribute("spx_voice.org_id", str(org_id))
...
class _OrgRoutingExporter(SpanExporter):
    ...
    Spans with a ``spx_voice.org_id`` attribute whose org has registered
...
def ensure_tracing() -> bool:
    ...
    try:
        ...
        logger.debug(f"Pipecat tracing utilities unavailable; tracing disabled: {exc}")
        return False
    ...
    setup_tracing(service_name="spx-voice-pipeline", exporter=_org_routing_exporter)
