...
from api.services.dograh_cloud import require_dograh_cloud_runtime
...
GOOGLE_REALTIME_INFERENCE_MODEL = "gemini-2.5-flash-lite"
OPENAI_REALTIME_INFERENCE_MODEL = "gpt-5-nano"
DEEPGRAM_FLUX_EOT_TIMEOUT_MS = 600
DEEPGRAM_ENDPOINTING_MS = 50
...
def create_stt_service(
    ...
    if user_config.stt.provider == ServiceProviders.DEEPGRAM.value:
        ...
        if user_config.stt.model == "flux-general-en":
            return DeepgramFluxSTTService(
                ...
                settings=DeepgramFluxSTTSettings(
                    ...
                    eot_timeout_ms=DEEPGRAM_FLUX_EOT_TIMEOUT_MS,
                    eot_threshold=0.7,
        ...
        require_dograh_cloud_runtime("Dograh STT provider")
...
def create_tts_service(user_config, audio_config: "AudioConfig"):
    ...
    if user_config.tts.provider == ServiceProviders.DEEPGRAM.value:
        ...
        require_dograh_cloud_runtime("Dograh TTS provider")
...
def create_llm_service_from_provider(
    ...
    *,
    ...
    if provider == ServiceProviders.OPENAI.value:
        ...
        require_dograh_cloud_runtime("Dograh LLM provider")
...
def create_realtime_llm_service(user_config, audio_config: "AudioConfig"):
    ...
    def google_realtime_latency_settings(model_name: str) -> dict:
    ...
    if provider == ServiceProviders.OPENAI_REALTIME.value:
        ...
        settings_kwargs = {
            "model": model,
            "voice": voice or "Puck",
            **google_realtime_latency_settings(model),
        }
        if language:
...
def create_realtime_inference_llm_service(user_config):
    """Create the side-channel text LLM used during realtime calls.

    Realtime speech models handle the main conversation. For extraction and
    other out-of-band text inference, prefer the same vendor/key as the realtime
    model so a stale default text LLM cannot block a realtime-only setup.
    """
    realtime_config = getattr(user_config, "realtime", None)
    ...
    if provider_value == ServiceProviders.GOOGLE_REALTIME.value:
        return create_llm_service_from_provider(
            ...
            GOOGLE_REALTIME_INFERENCE_MODEL,
            realtime_config.api_key,
        )

    if provider_value == ServiceProviders.OPENAI_REALTIME.value:
        return create_llm_service_from_provider(
            ServiceProviders.OPENAI.value,
            OPENAI_REALTIME_INFERENCE_MODEL,
            realtime_config.api_key,
        )

    if getattr(user_config, "llm", None) is not None:
        return create_llm_service(user_config)

    raise HTTPException(
        status_code=400,
        detail=(
            "Realtime side-channel inference needs a configured LLM for this "
            "realtime provider."
        ),
    )
