...
from api.services.pipecat.service_factory import (
    ...
    create_realtime_inference_llm_service,
...
LOW_LATENCY_USER_SPEECH_TIMEOUT_SECONDS = 0.25


def _fast_speech_timeout_stop_strategy() -> SpeechTimeoutUserTurnStopStrategy:
    return SpeechTimeoutUserTurnStopStrategy(
        user_speech_timeout=LOW_LATENCY_USER_SPEECH_TIMEOUT_SECONDS
    )
    ...
    if provider in {
        ...
        return (
            UserTurnStrategies(
                ...
                stop=[_fast_speech_timeout_stop_strategy()],
            ),
...
async def _run_pipeline(
    ...
    if is_realtime:
        ...
        # inference calls.
        inference_llm = create_realtime_inference_llm_service(user_config)
    else:
    ...
    if is_realtime:
        ...
        realtime_provider_value = (
            ...
            if isinstance(realtime_provider, ServiceProviders)
            else realtime_provider
        )
        llm_provider = getattr(user_config.llm, "provider", None)
        llm_model = getattr(user_config.llm, "model", None)
        if realtime_provider_value == ServiceProviders.GOOGLE_REALTIME.value:
            llm_provider = ServiceProviders.GOOGLE.value
            llm_model = GOOGLE_REALTIME_INFERENCE_MODEL
        elif realtime_provider_value == ServiceProviders.OPENAI_REALTIME.value:
            llm_provider = ServiceProviders.OPENAI.value
            llm_model = OPENAI_REALTIME_INFERENCE_MODEL
        elif isinstance(llm_provider, ServiceProviders):
            llm_provider = llm_provider.value

        runtime_configuration = {
            "realtime_provider": realtime_provider_value,
    ...
    # Resolve embeddings for knowledge-base retrieval. In realtime mode this can
    # reuse the Google realtime key, avoiding a separate embedding setup step.
