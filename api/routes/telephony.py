...
from api.constants import (
    DEFAULT_INBOUND_CONCURRENCY_LIMIT,
    INBOUND_QUEUE_MAX_WAIT_SECONDS,
    INBOUND_QUEUE_RETRY_SECONDS,
)
...
from api.services.telephony.inbound_queue import (
    InboundQueueAdmission,
    inbound_queue_manager,
)
    ...
    if is_livekit_runtime():
        ...
        if telephony_configuration_id:
            ...
            telephony_configuration_id = (
                telephony_config.id if telephony_config else None
            )

        sip_trunk_id: str | None = None
        from_number: str | None = None
        if telephony_config:
            if telephony_config.provider == WorkflowRunMode.VOBIZ.value:
                await sync_vobiz_livekit_config(
                    config_id=telephony_config.id,
                    organization_id=user.selected_organization_id,
                )
                telephony_config = (
                    await db_client.get_telephony_configuration_for_org(
                        telephony_config.id, user.selected_organization_id
                    )
                    or telephony_config
                )

            credentials = telephony_config.credentials or {}
            sip_trunk_id = credentials.get("livekit_sip_outbound_trunk_id")

            if request.from_phone_number_id is not None:
                phone_row = await db_client.get_phone_number_for_config(
                    request.from_phone_number_id, telephony_config.id
                )
                if not phone_row or not phone_row.is_active:
                    raise HTTPException(
                        status_code=400, detail="from_phone_number_not_found"
                    )
                from_number = phone_row.address_normalized
            else:
                phone_row = await db_client.get_default_caller_id(telephony_config.id)
                if not phone_row or not phone_row.is_active:
                    active_numbers = await db_client.list_phone_numbers_for_config(
                        telephony_config.id
                    )
                    phone_row = next((n for n in active_numbers if n.is_active), None)
                if phone_row:
                    from_number = phone_row.address_normalized
        ...
        if not workflow_run_id:
            ...
            if workflow_run.workflow_id != request.workflow_id:
                raise HTTPException(
                    status_code=400,
                    detail="Workflow run does not belong to this workflow",
                )
        ...
        try:
            livekit_call = await create_outbound_sip_call(
        ...
        await db_client.update_workflow_run(
            ...
            gathered_context={
                "provider": WorkflowRunMode.LIVEKIT.value,
                "call_id": livekit_call.call_id,
                "livekit_room": livekit_call.room_name,
                "livekit_dispatch_id": livekit_call.dispatch_id,
                "livekit_sip_participant_id": livekit_call.sip_participant_id,
            },
            initial_context=workflow_run.initial_context or {},
            cost_info={"call_id": livekit_call.call_id},
        )
        return {
            "message": f"LiveKit SIP call initiated successfully with run name {workflow_run_name}",
            "room_name": livekit_call.room_name,
            "call_id": livekit_call.call_id,
        }
...
def _configuration_int(value, default: int) -> int:
...
async def _get_inbound_concurrency_limit(organization_id: int) -> int:
    value = await db_client.get_configuration_value(
        organization_id,
        OrganizationConfigurationKey.INBOUND_CONCURRENT_CALL_LIMIT.value,
        default=DEFAULT_INBOUND_CONCURRENCY_LIMIT,
    )
    return _configuration_int(value, DEFAULT_INBOUND_CONCURRENCY_LIMIT)
...
async def _create_inbound_workflow_run(
    ...
    queue_admission: InboundQueueAdmission | None = None,
) -> int:
    ...
    workflow_run = await db_client.create_workflow_run(
        ...
        initial_context={
            ...
            **(
                {
                    "inbound_queue": queue_admission.metadata(
                        max_wait_seconds=INBOUND_QUEUE_MAX_WAIT_SECONDS,
                        retry_seconds=INBOUND_QUEUE_RETRY_SECONDS,
                    )
                }
...
async def _release_inbound_slot_for_run(workflow_run_id: int, workflow_run=None):
    released = await inbound_queue_manager.release_for_run(workflow_run_id)
    if released:
        logger.info(f"[run {workflow_run_id}] Released inbound active-call slot")
        return
    ...
    try:
        ...
        return
    if not call_id or organization_id is None:
        return

    released = await inbound_queue_manager.release_call(
        organization_id=organization_id,
        call_id=call_id,
        workflow_run_id=workflow_run_id,
    )
...
async def _handle_telephony_websocket(
    ...
    try:
        ...
        try:
            await _release_inbound_slot_for_run(workflow_run_id, workflow_run)
        except Exception as e:
            logger.warning(
                f"[run {workflow_run_id}] Failed to release inbound active-call slot: {e}"
            )
...
async def handle_inbound_run(request: Request):
    ...
    try:
        ...
        queue_admission: InboundQueueAdmission | None = None
        if (
            provider_class.PROVIDER_NAME == WorkflowRunMode.VOBIZ.value
            and normalized_data.call_id
        ):
            max_active_calls = await _get_inbound_concurrency_limit(
                config.organization_id
            )
            queue_admission = await inbound_queue_manager.admit_or_queue(
            ...
            if queue_admission.queued:
                return provider_instance.generate_queue_wait_response(
                    position=queue_admission.position,
                    redirect_url=webhook_url,
                    retry_seconds=INBOUND_QUEUE_RETRY_SECONDS,
                )

            if queue_admission.expired:
                return provider_instance.generate_queue_timeout_response()

        # 5. Create workflow run + return provider-shaped response.
        workflow_run_id = (
            queue_admission.workflow_run_id
            if queue_admission and queue_admission.workflow_run_id
            else None
        )
        if workflow_run_id is None:
            try:
                ...
                if queue_admission is not None:
                    await inbound_queue_manager.mark_run_admitted(
                        organization_id=config.organization_id,
                        call_id=normalized_data.call_id,
                        workflow_run_id=workflow_run_id,
                    )
                ...
                if queue_admission is not None:
                    await inbound_queue_manager.release_call(
                        organization_id=config.organization_id,
                        call_id=normalized_data.call_id,
                    )
                raise
