...
from api.services.dograh_cloud import require_dograh_cloud
...
async def create_workflow_from_template(
    ...
    """
    require_dograh_cloud("AI workflow generation")
            ...
            workflow_configurations = {
                **workflow_configurations,
                "model_overrides": normalize_model_overrides(
                    model_overrides,
                    is_realtime=model_overrides.get(
                        "is_realtime", user_config.is_realtime
                    ),
                ),
            }
            try:
                ...
                normalized_model_overrides = workflow_configurations["model_overrides"]
                services_to_validate = set(normalized_model_overrides).intersection(
                    SERVICE_FIELDS
                )
                if "is_realtime" in normalized_model_overrides:
                ...
                logger.exception("Unexpected workflow model override validation error")
...
async def create_workflow_run(
    ...
    if is_livekit_runtime() and request.mode in {"webrtc", "smallwebrtc"}:
        run_mode = "livekit"
