...
from api.services.livekit.client import is_livekit_runtime
...
def _sensitive_fields(provider_name: str) -> List[str]:
    ...
    if spec is None or spec.ui_metadata is None:
        ...
        fields = [f.name for f in spec.ui_metadata.fields if f.sensitive]
    if provider_name == "vobiz":
        fields.extend(VOBIZ_LIVEKIT_SENSITIVE_CREDENTIAL_FIELDS)
    return fields
...
async def _postprocess_telephony_configuration(
    row,
    *,
...
async def _sync_inbound_for_phone_number(
    ...
    if config and config.provider == "vobiz" and is_livekit_runtime():
        try:
            ...
            logger.error(
                f"Vobiz LiveKit sync raised for config {config_id} "
                f"address {address}: {e}"
            )
            return ProviderSyncStatus(
                ok=False, message=f"Vobiz LiveKit sync failed: {e}"
            )
...
async def create_telephony_configuration(
    ...
    requested_numbers = getattr(request.config, "from_numbers", []) or []
    ...
    row = await _postprocess_telephony_configuration(
        row,
        requested_phone_numbers=requested_numbers,
    )
...
async def update_telephony_configuration(
    ...
    if row and request.config is not None:
        row = await _postprocess_telephony_configuration(
            row,
            requested_phone_numbers=getattr(request.config, "from_numbers", []) or [],
        )
...
async def create_phone_number(
    ...
    if cfg.provider == "vobiz" and is_livekit_runtime():
...
async def delete_phone_number(
    ...
    if cfg.provider == "vobiz" and is_livekit_runtime():
        await _sync_inbound_for_phone_number(
            config_id, user.selected_organization_id, existing.address
        )
...
async def save_telephony_configuration(
    ...
    if default and default.provider == request.provider:
        ...
        if request.provider == "vobiz":
            if payload.get("auth_id") == (default.credentials or {}).get("auth_id"):
                payload = preserve_vobiz_livekit_credentials(
                    payload, default.credentials or {}
                )
            else:
                for field in VOBIZ_LIVEKIT_DERIVED_CREDENTIAL_FIELDS:
    ...
    row = await _postprocess_telephony_configuration(
        row,
        requested_phone_numbers=new_addresses,
    )
