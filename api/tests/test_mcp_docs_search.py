...
def fake_docs_root(tmp_path: Path) -> Path:
    ...
    (docs_root / "getting-started" / "index.mdx").write_text(
        ...
        'description: "Start using SPX Voice."\n'
        "---\n\n"
        "# Getting started\n\n"
        "Welcome to SPX Voice.\n",
        encoding="utf-8",
    )
    ...
    (docs_root / "deployment" / "docker.mdx").write_text(
        "---\n"
        'title: "Docker"\n'
        'description: "Deploy SPX Voice with Docker."\n'
...
""",
    ...
    _clear_docs_caches()
    with patch.dict(os.environ, {"SPX Voice_DOCS_PATH": str(docs_root)}):
        yield docs_root
    _clear_docs_caches()
...
def test_resolve_docs_root_honors_env_override(tmp_path: Path):
    ...
    with patch.dict(os.environ, {"SPX Voice_DOCS_PATH": str(docs)}):
...
async def test_search_docs_respects_limit(fake_docs_root, authed_user):
    results = await search_docs("SPX Voice", limit=1)
...
async def test_search_docs_returns_empty_when_no_corpus(
    ...
    monkeypatch.setenv("SPX Voice_DOCS_PATH", str(nonexistent))
    _clear_docs_caches()
    with patch(
...
async def test_search_docs_rejects_zero_limit(fake_docs_root, authed_user):
    with pytest.raises(ValueError, match="at least 1"):
        await search_docs("SPX Voice", limit=0)
...
async def test_read_doc_can_target_section(fake_docs_root, authed_user):
    ...
    assert "Run SPX Voice with Docker." not in result["content"]
