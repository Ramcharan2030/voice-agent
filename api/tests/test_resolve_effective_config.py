...
class TestAPIKeyInheritance:
    ...
    def test_blank_api_key_list_in_override_inherits_global(self, global_config):
        """Blank API key lists from forms should not replace a working key."""
        result = resolve_effective_config(
            global_config,
            {"llm": {"model": "gpt-4.1-mini", "api_key": []}},
        )
        assert result.llm.model == "gpt-4.1-mini"
        assert result.llm.api_key == "sk-global-llm"

    def test_normalize_model_overrides_removes_blank_api_keys(self):
        ...
        assert "api_key" not in overrides["llm"]
        assert "api_key" not in overrides["realtime"]

    def test_normalize_realtime_overrides_removes_inactive_traditional_sections(self):
        overrides = normalize_model_overrides(
            {
                "is_realtime": True,
                "stt": {
        ...
        assert "stt" not in overrides
        assert "tts" not in overrides
        assert overrides["realtime"]["api_key"] == "goog-override"

    def test_blank_api_key_string_in_override_inherits_global(self, global_config):
        result = resolve_effective_config(
            global_config,
            {"llm": {"model": "gpt-4.1-mini", "api_key": "  "}},
        )
...
class TestRealtimeOverride:
    ...
    def test_blank_realtime_api_key_inherits_global(self, global_config_realtime):
        result = resolve_effective_config(
            global_config_realtime,
            {"realtime": {"voice": "Kore", "api_key": []}},
        )
        assert result.realtime.voice == "Kore"
        assert result.realtime.api_key == "goog-global-rt"

    def test_realtime_ignores_incomplete_traditional_overrides(self):
        realtime_only_config = UserConfiguration(
            llm=OpenAILLMService(
                provider="openai",
                api_key="sk-global-llm",
                model="gpt-4.1",
            ),
            stt=None,
            tts=None,
