...
const formatUsdAmount = (amount: number) => {
    const precision = Math.abs(amount) < 0.01 && amount !== 0 ? 4 : 2;
    return `$${amount.toFixed(precision)}`;
};

const runUsdCost = (run: WorkflowRunUsageResponse) => {
    if (run.total_cost_usd !== undefined && run.total_cost_usd !== null) {
...
export default function UsagePage() {
    ...
    const { userConfig, saveUserConfig, loading: userConfigLoading } = useUserConfig();
    ...
    const showUsdCosts = Boolean(usageHistory?.runs.some(run => runUsdCost(run) !== null));
    ...
    const fetchMpsCredits = useCallback(async () => {
        if (!auth.isAuthenticated || !hostedServicesEnabled) {
    ...
    }, [auth.isAuthenticated, hostedServicesEnabled]);
    ...
    }, [auth.isAuthenticated, currentPage, appliedFilters, fetchUsageHistory, fetchMpsCredits, fetchDailyUsage, hostedServicesEnabled]);
    ...
    return (
        <div className="container mx-auto px-4 py-8 space-y-8">
            ...
            <div className="relative animate-fade-in-up">
                <div className="absolute -top-10 -right-10 w-64 h-64 bg-gradient-to-br from-emerald-500/5 to-teal-500/5 rounded-full blur-3xl -z-10" />
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">View all your agent runs across all voice agents</p>
                    </div>
                ...
                <Card className="card-hover overflow-hidden">
                    <CardHeader className="pb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <CardTitle className="text-lg">Managed Model Credits</CardTitle>
                                <CardDescription>
                                    Track usage of hosted model providers using service keys
                                </CardDescription>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent>
                        {isLoadingCredits ? (
                            ...
                            <p className="text-muted-foreground">No service keys configured. Set up a service key in your model configuration to see usage.</p>
                        )}
                    </CardContent>
                </Card>
                )}
                                            ...
                                            {usageHistory.runs.map((run) => (
                                                ...
                                                >
                                                    ...
                                                    <TableCell className="text-right font-medium">
                                                        {runUsdCost(run) !== null
                                                            ? formatUsdAmount(runUsdCost(run) as number)
