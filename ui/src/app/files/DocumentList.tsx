...
import {
  ...
  retryDocumentProcessingApiV1KnowledgeBaseDocumentsDocumentUuidRetryPost,
} from '@/client/sdk.gen';
...
export default function DocumentList({ refreshTrigger }: DocumentListProps) {
  ...
  const [retryingDocumentUuid, setRetryingDocumentUuid] = useState<string | null>(null);
  ...
  const handleRetry = async (doc: DocumentResponseSchema) => {
    try {
      setRetryingDocumentUuid(doc.document_uuid);
      const response =
        await retryDocumentProcessingApiV1KnowledgeBaseDocumentsDocumentUuidRetryPost({
          path: {
      ...
      toast.success(`Retrying "${doc.filename}"`);
      fetchDocuments();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to retry document');
      logger.error('Error retrying document:', err);
    } finally {
      setRetryingDocumentUuid(null);
    }
  };

  const getStatusAgeMs = (doc: DocumentResponseSchema) => {
    const updatedAt = new Date(doc.updated_at).getTime();
    if (!Number.isFinite(updatedAt)) return 0;
    return Date.now() - updatedAt;
  };

  const isStaleStatus = (doc: DocumentResponseSchema) =>
    (doc.processing_status === 'processing' || doc.processing_status === 'pending') &&
    getStatusAgeMs(doc) > STALE_PROCESSING_MS;

  const canRetry = (doc: DocumentResponseSchema) =>
    doc.processing_status === 'failed' || isStaleStatus(doc);

  const getStatusBadge = (doc: DocumentResponseSchema) => {
    if (isStaleStatus(doc)) {
  ...
  const getDocumentStatusText = (doc: DocumentResponseSchema) => {
    if (isStaleStatus(doc)) {
      return 'Indexing has been waiting longer than expected';
    }
    if (doc.processing_status === 'pending') {
      return 'Queued for indexing';
    }
    if (doc.processing_status === 'processing') {
      return doc.retrieval_mode === 'full_document'
        ? 'Extracting document text'
        : 'Building vector search index';
    }
    if (doc.processing_status === 'completed') {
      return doc.retrieval_mode === 'full_document'
        ? 'Ready for full-document retrieval'
              ...
              <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground">
                    ...
                    {getDocumentStatusText(doc) && <span>{getDocumentStatusText(doc)}</span>}
              ...
              <div className="flex items-center gap-2 shrink-0">
                {canRetry(doc) && (
                  <Button
                    ...
                    disabled={retryingDocumentUuid === doc.document_uuid}
                  >
                    <RefreshCw
                      className={`w-4 h-4 ${
                        retryingDocumentUuid === doc.document_uuid ? 'animate-spin' : ''
                      }`}
                    />
                    Retry
                  </Button>
