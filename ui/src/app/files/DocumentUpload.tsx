...
const ACCEPTED_FILE_TYPES = ['.pdf', '.docx', '.txt', '.md', '.csv', '.json', '.html'];

export default function DocumentUpload({ onUploadSuccess }: DocumentUploadProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [retrievalMode, setRetrievalMode] = useState<string>('chunked');
        ...
        <div className="space-y-3">
          ...
          <RadioGroup value={retrievalMode} onValueChange={setRetrievalMode}>
            ...
            >
              ...
              <div>
                <p className="font-medium text-sm">Fast Vector Search</p>
                <p className="text-xs text-muted-foreground">
                  Splits the document into searchable chunks and retrieves only the most relevant facts.
                  Best for low-latency voice calls and large policy, FAQ, or scheme documents.
                </p>
              </div>
            </label>
            <label
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          Supported formats: {ACCEPTED_FILE_TYPES.join(', ')} (Max 25MB)
        </p>
      </div>
