                ...
                const response = await createOrUpdateEmbedTokenApiV1WorkflowWorkflowIdEmbedTokenPost({
                    ...
                    body: {
                        ...
                        settings: {
                            ...
                            size: "medium",
                            autoStart: false,
                            containerId: embedMode === "inline" ? "voice-agent-inline-container" : undefined,
                        },
                        usage_limit: null,
                                        ...
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                ...
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    ...
                                                    <li>• Call <code className="text-xs">window.VoiceAgentWidget.start()</code> to begin a call.</li>
                                                    <li>• Call <code className="text-xs">window.VoiceAgentWidget.end()</code> to end it.</li>
...
window.VoiceAgentWidget?.onStatusChange((status) => {
  callStatus = status;
...
document.getElementById('talk-btn').addEventListener('click', () => {
  if (callStatus === 'connected' || callStatus === 'connecting') {
    window.VoiceAgentWidget.end();
  } else {
    window.VoiceAgentWidget.start();
  }
});`}</code>
                                                </pre>
  ...
  useEffect(() => {
    window.VoiceAgentWidget?.onStatusChange(setStatus);
  }, []);

  const isLive = status === 'connected' || status === 'connecting';
  return (
    <button onClick={() => isLive ? window.VoiceAgentWidget.end() : window.VoiceAgentWidget.start()}>
                                    ...
                                    {embedMode === "inline" && (
                                        <div className="space-y-3">
                                            <div className="rounded-lg bg-muted/50 p-4">
                                                <h4 className="font-medium mb-2">Integration Instructions</h4>
                                                <ul className="text-sm space-y-2 text-muted-foreground">
                                                    <li>• Add a div with id=&quot;voice-agent-inline-container&quot; where you want the widget</li>
                                                    <li>• The widget will render inside this container</li>
                                                    <li>• You have full control over the container&apos;s styling</li>
                                                    <li>• Call window.VoiceAgentWidget.start() to begin the call</li>
                                                    <li>• Call window.VoiceAgentWidget.end() to end the call</li>
                                                </ul>
                                            </div>
                                            ...
                                            <div className="rounded-lg bg-blue-50 dark:bg-blue-950/20 p-4 border border-blue-200 dark:border-blue-800">
                                                ...
                                                <pre className="text-xs overflow-x-auto">
                                                    <code className="text-blue-800 dark:text-blue-200">{`export function VoiceAgent() {
  const [isCallActive, setIsCallActive] = useState(false);

  useEffect(() => {
    // Widget will auto-initialize when script loads
    window.VoiceAgentWidget?.onCallStart(() => {
      setIsCallActive(true);
    });
    window.VoiceAgentWidget?.onCallEnd(() => {
  ...
  return (
    <div className="my-8">
      ...
      <div id="voice-agent-inline-container" className="min-h-[400px]">
        {/* Widget renders here */}
      </div>
      <button
        onClick={() => window.VoiceAgentWidget?.start()}
