...
function formatApiErrorDetail(detail: unknown, fallback: string): string {
    ...
    if (Array.isArray(detail)) {
        const messages = detail.map((entry) => {
            ...
            const location = Array.isArray(e.loc)
                ? e.loc.filter((part) => part !== "body").join(".")
                : "";
            const message = e.model && e.message
                ? `${e.model}: ${e.message}`
    ...
    if (detail && typeof detail === "object") {
        const nested = detail as { detail?: unknown; message?: unknown; errors?: unknown };
        if (typeof nested.message === "string") return nested.message;
        if (nested.errors) return formatApiErrorDetail(nested.errors, fallback);
        if (nested.detail) return formatApiErrorDetail(nested.detail, fallback);
    ...
    const saveWorkflowConfigurations = useCallback(async (configurations: WorkflowConfigurations, newWorkflowName: string) => {
        ...
        try {
            ...
            if (response.error) {
                ...
                const msg = formatApiErrorDetail(detail, 'Failed to save workflow configurations');
                throw new Error(msg);
            }
