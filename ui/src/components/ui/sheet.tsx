    ...
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          ...
          "fixed z-50 flex flex-col shadow-xl transition ease-out",
      ...
      >
        {children}
        <SheetPrimitive.Close className="ring-offset-background focus:ring-ring data-[state=open]:bg-secondary absolute top-4 right-4 rounded-xl opacity-70 transition-all duration-200 hover:opacity-100 hover:bg-accent focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none p-2">
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col gap-1.5 p-6 pb-4 border-b", className)}
      {...props}
    />
  )
}
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("mt-auto flex flex-col gap-2 p-6 pt-4 border-t", className)}
      {...props}
    />
  )
}
