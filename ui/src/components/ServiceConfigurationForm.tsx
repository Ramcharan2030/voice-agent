...
type ApiKeyValue = string | string[] | null | undefined;
...
export function ServiceConfigurationForm({
    ...
    const hostedServicesEnabled = Boolean(appConfig?.hostedServicesEnabled);
    ...
    useEffect(() => {
        const fetchConfigurations = async () => {
            ...
            const filterProviderSchemas = (items: Record<string, ProviderSchema>) => {
                if (hostedServicesEnabled) return items;
                const rest = { ...items };
                delete rest.dograh;
                return rest;
            };
            const selectProvider = (preferred: string, options: Record<string, ProviderSchema>) => {
                if (preferred && options[preferred]) return preferred;
                return Object.keys(options)[0] || "";
            };

            const llmSchemas = filterProviderSchemas(response.data.llm as Record<string, ProviderSchema>);
            const ttsSchemas = filterProviderSchemas(response.data.tts as Record<string, ProviderSchema>);
            const sttSchemas = filterProviderSchemas(response.data.stt as Record<string, ProviderSchema>);
            const embeddingsSchemas = response.data.embeddings as Record<string, ProviderSchema>;
            ...
            const selectedProviders: Record<ServiceSegment, string> = {
                llm: selectProvider(response.data.default_providers.llm, llmSchemas),
            ...
            const setServicePropertyValues = (service: ServiceSegment) => {
                ...
                const schemaSource = service === "realtime"
                    ...
                    : ({
                        ...
                        stt: sttSchemas,
                        embeddings: embeddingsSchemas,
                    }[service as "llm" | "tts" | "stt" | "embeddings"]);

                if (src?.provider && schemaSource?.[src.provider as string]) {
            ...
            const allSchemas = {
                ...
                embeddings: embeddingsSchemas,
                realtime: realtimeSchemas,
            } as Record<string, Record<string, ProviderSchema>>;
    ...
    }, [reset, configSource, hostedServicesEnabled]);

    useEffect(() => {
        setActiveTab(isRealtime ? "realtime" : "llm");
    }, [isRealtime]);
    ...
    const getGlobalServiceConfig = (service: ServiceSegment) => {
        return (userConfig as Record<string, unknown> | null)?.[service] as Record<string, unknown> | undefined;
    };

    const providerRequiresApiKey = (service: ServiceSegment, provider: string): boolean => {
        if (!provider || PROVIDERS_WITHOUT_API_KEYS.has(provider)) return false;
        const providerSchema = schemas?.[service]?.[provider];
        if (!providerSchema) return true;
        return Boolean(providerSchema.properties.api_key);
    };

    const hasEnteredApiKey = (service: ServiceSegment): boolean => {
        return apiKeys[service].some(key => key.trim().length > 0);
    };

    const hasGlobalApiKeyForProvider = (service: ServiceSegment, provider: string): boolean => {
        const globalConfig = getGlobalServiceConfig(service);
    ...
    const getRealtimeOverrideError = (): string | null => {
        ...
        if (!enabledOverrides.realtime) {
            const globalRealtimeConfig = getGlobalServiceConfig("realtime");
            const globalProvider = globalRealtimeConfig?.provider as string | undefined;
            if (!globalProvider) {
                return "Realtime mode needs a realtime model. Enable the Realtime Model override and enter that provider's credentials, or configure a global realtime model first.";
            }
            if (
                providerRequiresApiKey("realtime", globalProvider) &&
                !hasConfiguredApiKey(globalRealtimeConfig?.api_key as ApiKeyValue)
            ) {
                return "The global realtime model is missing an API key. Enable the Realtime Model override and enter a key, or add the key in global model settings.";
            }
            return null;
        }
        ...
        if (
            ...
            !hasGlobalApiKeyForProvider("realtime", realtimeProvider)
        ) {
            return "Enter a Realtime Model API key, or use the same provider as a global realtime model with a saved key.";
        }
        return null;
    };

    const getRealtimeGlobalError = (): string | null => {
        if (mode !== 'global' || !isRealtime) return null;
        ...
        if (activeTab === "llm" && providerRequiresApiKey("llm", serviceProviders.llm) && !hasEnteredApiKey("llm")) {
            return "Enter an LLM API key before saving the LLM configuration.";
        }
        return null;
    };
    ...
    const onSubmit = async (data: FormValues) => {
        setApiError(null);
        const realtimeError = getRealtimeOverrideError() ?? getRealtimeGlobalError();
        if (realtimeError) {
            setApiError(realtimeError);
            return;
        }
        setIsSaving(true);
        ...
        try {
            if (mode === 'override') {
                ...
                // Global saves are intentionally tab-scoped so editing one
                // provider does not revalidate stale credentials from another.
                const saveConfig: Record<string, unknown> = {};
                const globalIsRealtime = !!(userConfig as Record<string, unknown> | null)?.is_realtime;
                if (activeTab === "realtime" || isRealtime !== globalIsRealtime) {
                    saveConfig.is_realtime = isRealtime;
                }

                const provider = serviceProviders[activeTab];
                if (providerRequiresApiKey(activeTab, provider) && !hasEnteredApiKey(activeTab)) {
                    setApiError(`Enter a ${activeTab === "stt" ? "Transcriber" : activeTab === "tts" ? "Voice" : activeTab === "llm" ? "LLM" : activeTab === "embeddings" ? "Embedding" : "Realtime Model"} API key before saving.`);
                    return;
                }
                saveConfig[activeTab] = buildServiceConfig(activeTab, data);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex items-center justify-between mb-4 p-4 border rounded-lg">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                        Uses a single speech-to-speech model with no separate STT/TTS pipeline.
                    </p>
                </div>
                <Switch
                    id="realtime-toggle"
            ...
            <Card>
                <CardContent className="pt-6">
                    <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as ServiceSegment)} className="w-full">
