...
export async function impersonateAsSuperadmin(params: {
  ...
  // served from the target application sub-domain. The route will set the
  ...
  // a superadmin sub-domain we want to switch to the app sub-domain. For any other
  // domain (e.g. localhost, staging, or already on the app) we just keep the same origin.
