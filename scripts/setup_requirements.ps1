...
# Install SPX Voice API requirements first so pipecat's extras win on any
# shared transitive dependencies (matches api/Dockerfile and CI workflow).
Write-Host "Installing SPX Voice API requirements..."
pip install -r api/requirements.txt

if ($Dev) {
    Write-Host "Installing SPX Voice API dev requirements..."
