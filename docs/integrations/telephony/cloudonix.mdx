...
description: "Configure Cloudonix for voice communication in SPX Voice"
---

## Overview

Cloudonix is a cloud communications platform that enables SPX Voice agents to connect to SIP communications
...
- A [Cloudonix account](https://cockpit.cloudonix.io/onboarding?affiliate=SPX Voice)
- A Cloudonix domain UUID (or the domain name)
- A Cloudonix domain API Key (Bearer Token)
- A Cloudonix **Voice Application** on that domain (optional — leave the field blank in SPX Voice and we'll auto-create one for you on save, with the application `url` pre-set)
- A Cloudonix outbound voice trunk service provider connection
- SPX Voice instance running and accessible

## Video Tutorial

Watch this step-by-step guide to set up Cloudonix with SPX Voice:

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/qLKX0F99jpU?si=tA3zwCsNRKe-WCI1"
  title="SPX Voice Cloudonix Setup"
...
1. Log in to your [Cloudonix Console](https://cockpit.cloudonix.io/onboarding?affiliate=SPX Voice)
2. Find your **Domain ID** (UUID or domain name) and **Bearer Token** (Domain API Key) on the dashboard
3. (Optional) Navigate to your domain's **Applications** and create (or open) the application you'll use with SPX Voice, then copy its **Application Name**. Skip this if you want SPX Voice to auto-create the Voice Application for you on save.

### Step 2: Configure in SPX Voice
...
4. Enter your credentials:
   - Bearer Token
   - Domain ID
   - Application Name — *optional*. Leave blank and SPX Voice will auto-create a Voice Application on save (with the application `url` and CXML runtime already configured) and store its name on this configuration.
...
6. Open the configuration you just created and add at least one **phone number** (with country code in E.164 format, e.g. `+1234567890`). The default caller ID is used for outbound calls.
   ...
   <Note>
     If SPX Voice auto-created the Voice Application for you, you still need to
     ...
     The auto-created application is named `SPX Voice-<random>` — its name is
...
Cloudonix routes inbound calls per **Voice Application** — the webhook URL is set once on the application, and applies to every DNID bound to it. **When you save an inbound workflow on a phone number, SPX Voice automatically pushes the webhook URL to your Voice Application's `url`** (provided the credentials are correct), so you don't need to set the webhook by hand. If SPX Voice auto-created the application during configuration save, the `url` is already set and this push is a no-op — you only need to bind your DNIDs to the auto-created application.
...
2. Create a new application (or open the existing one whose name you configured in SPX Voice) with these settings:
   - **Application Resource Type**: `Remote Application Resource`
   - **Application Runtime**: `Cloudonix (CXML)`
   - **Application URL**: `https://your-spx-voice-domain/api/v1/telephony/inbound/run`
   - **HTTP Method**: `POST`
3. Under **DNID Numbers**, add each phone number (DNID) you want to route through this application
4. Save

   <Note>
     The Application URL is what SPX Voice's auto-push updates in Step 3 — you
     ...
     Either works. Self-hosted SPX Voice deployments use their backend domain
     in place of `your-spx-voice-domain`.
   </Note>

   <img
     className="block dark:hidden"
     src="/images/cloudonix-inbound-config-application.png"
     alt="Cloudonix application form showing Remote Application Resource type, Cloudonix CXML runtime, the SPX Voice inbound URL, POST method, and a DNID number entry"
   />
   <img
     className="hidden dark:block"
     src="/images/cloudonix-inbound-config-application.png"
...
### Step 3: Assign an Inbound Workflow to the Phone Number in SPX Voice
...
2. Open the application whose name you configured in SPX Voice
3. Confirm:
   - **URL** is set to: `https://your-spx-voice-domain/api/v1/telephony/inbound/run`
   - **Method** is `POST`

   <Note>
     SPX Voice pushed this URL automatically when you saved the inbound workflow
     in Step 3. The same URL is shared across every DNID bound to that
     application — SPX Voice routes each inbound call to the right agent based
     on the called number's inbound workflow assignment. If the field is
     empty, shows a different URL, or SPX Voice surfaced a sync warning on
     save, the auto-push failed — most often because the Bearer Token,
     Domain ID, or Application Name in SPX Voice is incorrect. Paste the URL
     into the field yourself, set the method to `POST`, and save. On
     self-hosted SPX Voice, replace `your-spx-voice-domain` with your backend domain.
   </Note>
...
- Ensure your SPX Voice instance is publicly accessible
...
2. Verify your SPX Voice voice agent answers and responds
3. Check call logs in both SPX Voice dashboard and Cloudonix Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    ...
    configured in SPX Voice - Confirm the called number exists in your SPX Voice
    telephony configuration and has an **Inbound workflow** assigned -
    After assigning the inbound workflow, confirm SPX Voice successfully updated
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    /telephony-configurations - Verify the Bearer Token in SPX Voice matches the
...
- Use a dedicated Voice Application for SPX Voice so the shared `url` doesn't conflict with other systems
