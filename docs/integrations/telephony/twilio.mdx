...
description: "Configure Twilio for voice communication in SPX Voice"
---

## Overview

Twilio is a cloud communications platform that enables voice calling, messaging, and video capabilities. SPX Voice's Twilio integration provides seamless connectivity for your voice agents.
...
Inbound routing is driven by the phone number itself — there is a **single webhook URL** for the entire org, and SPX Voice resolves which agent to run from the called number's assigned inbound workflow. **When you save an inbound workflow on a phone number, SPX Voice automatically pushes the webhook URL to that number's `VoiceUrl` in your Twilio account** (provided the credentials are correct and the number belongs to that account).
...
3. In the **Voice Configuration** section, confirm:
   - **Webhook** is set to: `https://your-spx-voice-domain/api/v1/telephony/inbound/run`
   - **HTTP Method** is `POST`

   <Note>
     SPX Voice pushed this URL automatically when you saved the inbound workflow
     in Step 1. If the field is empty, shows a different URL, or SPX Voice
     ...
     `POST`, and click **Save Configuration**. On self-hosted SPX Voice,
...
3. Check call logs in both SPX Voice dashboard and Twilio Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls go to voicemail">
    ...
    exists in your SPX Voice telephony configuration and has an **Inbound
    workflow** assigned - Verify SPX Voice instance is running and responding
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    (Auth Token in SPX Voice matches Twilio Console) - Verify WebSocket connection
