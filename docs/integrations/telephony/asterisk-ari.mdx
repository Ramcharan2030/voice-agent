...
description: "Connect SPX Voice to your Asterisk PBX using the Asterisk REST Interface (ARI)"
---

## Overview

Asterisk ARI (Asterisk REST Interface) allows you to connect SPX Voice voice agents to your existing Asterisk PBX. ARI provides a WebSocket-based event model for controlling calls via Stasis applications, giving SPX Voice full control over call flow and audio streaming.

This guide focuses on the SPX Voice-specific configuration. For general Asterisk installation and administration, refer to the [official Asterisk documentation](https://docs.asterisk.org/).
...
- Network connectivity between your SPX Voice instance and Asterisk
- SPX Voice instance running and accessible

<Note>
If you compiled Asterisk from source, ensure `chan_websocket` is included during the build. This module is required for external media streaming between Asterisk and SPX Voice. Refer to the [Asterisk build system documentation](https://docs.asterisk.org/) for details on enabling modules.
</Note>

## Asterisk Configuration

The following Asterisk configuration files need to be set up to work with SPX Voice. These are minimal examples focused on the SPX Voice integration -- refer to the [Asterisk documentation](https://docs.asterisk.org/) for full configuration details.

### Enable ARI (`ari.conf`)

Create an ARI user that SPX Voice will use to authenticate:
...
<Note>
The username (section name, e.g., `SPX Voice`) and password here must match the **Stasis App Name** and **App Password** you configure in SPX Voice.
</Note>
...
Route incoming calls to your Stasis application so SPX Voice can handle them:

```ini
[from-external]
exten => _X.,1,NoOp(Incoming call to ${EXTEN})
 same => n,Stasis(SPX Voice)
 same => n,Hangup()
```

Replace `SPX Voice` with the app name you configured in `ari.conf` and in SPX Voice.

### Configure External Media Streaming (`websocket_client.conf`)

SPX Voice uses Asterisk's external media streaming to send and receive audio over WebSocket. Configure a WebSocket client connection that points to your SPX Voice instance:

```ini
[SPX Voice]
type = websocket_client
uri = ws://your-spx-voice-host:port/api/v1/telephony/ws/ari
protocols = audio
```

<Note>
The section name (e.g., `SPX Voice`) is the **WebSocket Client Name** you'll enter in the SPX Voice telephony configuration. This name tells Asterisk which WebSocket connection to use for external media streaming during calls.
</Note>
...
| **Stasis App Name** | The ARI username configured in `ari.conf` | `SPX Voice` |
| **App Password** | The ARI password configured in `ari.conf` | `your_secure_password` |
| **WebSocket Client Name** | The connection name from `websocket_client.conf` | `SPX Voice` |
...
Unlike other telephony providers that use HTTP webhooks for inbound calls, ARI delivers inbound calls as **StasisStart events on the ARI WebSocket**. SPX Voice automatically detects these events and activates the workflow assigned to the called extension.

### How It Works

1. An external call arrives at Asterisk and the dialplan routes it to `Stasis(SPX Voice)`
...
3. SPX Voice looks up the called extension in your telephony configuration's phone numbers, finds the assigned workflow, validates quota, and creates a workflow run
...
…or use a pattern that catches every extension you'll register in SPX Voice:
...
**Step 2: Add the extension as a phone number in SPX Voice**
...
4. Save
   ...
   <Note>
   Adding the extension in SPX Voice doesn't change Asterisk's dialplan — that's
   what Step 1 is for. The SPX Voice entry tells the StasisStart handler which
...
<AccordionGroup>
  <Accordion title="Cannot connect to ARI endpoint">
    - Verify the ARI endpoint URL is correct and reachable from your SPX Voice instance
  ...
  <Accordion title="No audio during calls">
    ...
    - Check that `websocket_client.conf` is correctly configured with the right SPX Voice URI
    - Ensure the WebSocket Client Name in SPX Voice matches the section name in `websocket_client.conf`
    - Verify network connectivity and firewall rules allow WebSocket traffic between Asterisk and SPX Voice
  </Accordion>

  <Accordion title="Calls not reaching SPX Voice">
  ...
  <Accordion title="Inbound calls are immediately hung up">
    ...
    - Review SPX Voice logs for warnings like "no matching phone number registered
    for config" or "has no inbound_workflow_id assigned"
  </Accordion>

  <Accordion title="WebSocket client connection issues">
    - Check the URI in `websocket_client.conf` points to the correct SPX Voice host and port
    - Verify the SPX Voice instance is running and accepting WebSocket connections
...
- Keep your Asterisk instance on the same network or a low-latency connection to SPX Voice for optimal audio quality
- Use strong passwords for ARI authentication
- Restrict ARI access to known IP addresses using firewall rules
- Monitor Asterisk logs alongside SPX Voice logs when debugging call issues
