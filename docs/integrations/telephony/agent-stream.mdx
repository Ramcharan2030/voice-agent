---
title: "Agent Stream"
description: "Stream audio to a SPX Voice agent over a WebSocket"
---

## Overview

Agent Stream is a WebSocket endpoint that lets a telephony provider point its media stream at a single URL and drive a SPX Voice agent run. The agent UUID in the URL selects the agent; provider-specific identifiers in the query string (for Cloudonix: `Domain`) tell SPX Voice which stored telephony configuration to use for that call. The bearer token and other credentials are never passed in the URL — they live in the stored configuration and are used by SPX Voice to validate the session and to issue provider API calls (hangup, transfer) during the call.

This is useful when:

- You're integrating SPX Voice into a SIP gateway or in-house dialer that already speaks a supported provider's streaming protocol
...
<Warning>
  ...
  request on the project issue tracker
  with your use case.
</Warning>
...
```
wss://your-spx-voice-domain/api/v1/agent-stream/{agent_uuid}
```

`{agent_uuid}` is the agent's stable UUID (see [Get the Agent UUID](#get-the-agent-uuid) below). On self-hosted deployments, replace `your-spx-voice-domain` with your backend host.

## Prerequisites

- A SPX Voice agent (workflow) — published or in draft is fine
- A Cloudonix telephony configuration in your SPX Voice organization whose `domain_id` matches the `Domain` you pass on the URL. SPX Voice uses the bearer token from this configuration to validate the call session and to issue provider API calls (hangup, transfer).
...
| `Domain` | Yes (cloudonix) | Cloudonix domain ID. SPX Voice uses this to look up the matching stored telephony configuration and retrieve the bearer token used for provider API calls. |
...
Use this URL inside the CXML `<Stream>` your Cloudonix Voice Application returns when the call needs to be bridged to the SPX Voice agent:

```xml
<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Connect>
    <Stream url="wss://your-spx-voice-domain/api/v1/agent-stream/{agent_uuid}?provider=cloudonix&Domain=...&callId=...&from=...&to=..."/>
  </Connect>
  <Pause length="40"/>
</Response>
```

The first two messages on the socket should be Cloudonix's standard `connected` and `start` events (Twilio-compatible framing). SPX Voice extracts `streamSid` and `callSid` from the `start` event payload, validates the session against Cloudonix using the bearer token from the stored telephony configuration matched by `Domain`, and then begins streaming audio.

## Workflow run lifecycle

When the WebSocket is accepted, SPX Voice:
...
- No bearer tokens or provider secrets are passed in the URL. Provider credentials live in the stored telephony configuration (matched by `Domain` for Cloudonix) and are used server-side by SPX Voice to validate the session and issue provider API calls.
