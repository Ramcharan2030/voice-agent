...
description: "Trigger a SPX Voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the SPX Voice UI.

## Prerequisites

- A SPX Voice [API key](/configurations/api-keys) exported as `SPX_VOICE_API_KEY`
...
from spx_voice_sdk._generated_models import InitiateCallRequest

with SPXVoiceClient(api_key="YOUR_API_KEY") as client:
...
const client = new SPXVoiceClient({ apiKey: "YOUR_API_KEY" });
