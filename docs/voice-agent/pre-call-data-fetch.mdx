...
| Field | Description |
| --- | --- |
| **Endpoint URL** | The URL SPX Voice will send the POST request to. |
...
SPX Voice sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/SPX Voice/pre-call", async (req, res) => {
