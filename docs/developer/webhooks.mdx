...
description: "Context variables available in webhook nodes and the data SPX Voice sends after a call"
---

SPX Voice executes **webhook nodes** asynchronously after a workflow run completes. You configure the target URL, HTTP method, headers, and payload template directly in the workflow definition.
...
2. SPX Voice executes any `webhook` nodes in the workflow asynchronously
...
@app.post("/webhook/SPX Voice")
async def handle_spx_voice_webhook(request: Request):
...
```json
{
  ...
  "data": {
    ...
    "enabled": true,
    "http_method": "POST",
    "endpoint_url": "https://your-service.com/webhook/SPX Voice",
    "payload_template": {
      "run_id": "{{workflow_run_id}}",
      "customer": "{{initial_context.customer_name}}",
      "outcome": "{{gathered_context.resolution}}"
