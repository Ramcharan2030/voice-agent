...
sequenceDiagram
    ...
    participant API as SPX Voice API
...
**Telephony**
The phone infrastructure. SPX Voice connects to your telephony provider (Twilio, Vonage, etc.) to place and receive calls. The audio streams between the caller and SPX Voice in real time.

**Transcriber (STT)**
Converts the caller's speech to text in real time. SPX Voice sends the audio stream to your configured speech-to-text provider and uses the transcript to drive both the LLM and the final run record.
...
1. SPX Voice instructs your telephony provider to dial the number
...
6. When an edge condition is met, SPX Voice transitions to the next node
